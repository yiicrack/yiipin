...
class SysmsgController extends Controller
{
	...
	{
		$form = new Form( "backend.views.sysmsg.formConfig" );
		$form->model = new Sysmsg( );
		...
		if ( $form->submitted( "btnsubmit" ) && $form->validate( ) )
		{
			...
			if ( $model->save( FALSE ) )
			{
				Yii::app( )->user->setFlash( "success", Sysmsg::model( )->modelName.( "“".$model."”已成功添加！" ) );
				$this->redirect( array(
					"view",
					"id" => $model->id
				) );
	...
	public function actionUpdate( )
	{
		...
		if ( $form->submitted( "btnsubmit" ) && $form->validate( ) )
		{
			...
			if ( $model->save( FALSE ) )
			{
				Yii::app( )->user->setFlash( "success", Sysmsg::model( )->modelName.( "“".$model."”已成功修改！" ) );
				$this->redirect( array(
					"view",
					"id" => $model->id
				) );
	...
	public function actionOperate( )
	{
		if ( Yii::app( )->request->isAjaxRequest && !empty( $_POST['id'] ) )
		{
			...
			{
				$models = Sysmsg::model( )->findAllByPk( $_POST['id'] );
				if ( $models )
				{
	...
	{
		if ( isset( $_POST['ajax'] ) && $_POST['ajax'] === "sysmsg-form" )
		{
