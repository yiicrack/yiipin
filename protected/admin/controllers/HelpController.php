...
class HelpController extends Controller
{
	...
	{
		$form = new Form( "backend.views.help.formConfig" );
		$form->model = new Help( );
		...
		if ( $form->submitted( "btnsubmit" ) && $form->validate( ) )
		{
			...
			if ( $model->save( FALSE ) )
			{
				Yii::app( )->user->setFlash( "success", Help::model( )->modelName.( "“".$model."”已成功添加！" ) );
				$this->redirect( array(
					"view",
					"id" => $model->id
				) );
	...
	public function actionUpdate( )
	{
		...
		if ( $form->submitted( "btnsubmit" ) && $form->validate( ) )
		{
			...
			if ( $model->save( FALSE ) )
			{
				Yii::app( )->user->setFlash( "success", Help::model( )->modelName.( "“".$model."”已成功修改！" ) );
				$this->redirect( array(
					"view",
					"id" => $model->id
				) );
	...
	public function actionOperate( )
	{
		if ( Yii::app( )->request->isAjaxRequest && !empty( $_POST['id'] ) )
		{
			...
			{
				$models = Help::model( )->findAllByPk( $_POST['id'] );
				if ( $models )
				{
