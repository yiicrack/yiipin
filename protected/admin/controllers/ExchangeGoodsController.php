...
class ExchangeGoodsController extends Controller
{
	...
	{
		$form = new Form( "backend.views.exchangeGoods.formConfig" );
		$form->model = new ExchangeGoods( );
		...
		if ( $form->submitted( "btnsubmit" ) )
		{
			...
			if ( $form->validate( ) )
			{
				...
				if ( $model->save( FALSE ) )
				{
					Yii::app( )->user->setFlash( "success", ExchangeGoods::model( )->modelName.( "“".$model."”已成功添加！" ) );
					$this->redirect( array(
						"view",
						"id" => $model->id
					) );
		...
		if ( $form->submitted( "btnsubmit" ) )
		{
			...
			if ( $form->validate( ) )
			{
				...
				if ( $model->save( FALSE ) )
				{
					...
					Yii::app( )->user->setFlash( "success", ExchangeGoods::model( )->modelName.( "“".$model."”已成功修改！" ) );
					$this->redirect( array(
						"view",
						"id" => $model->id
					) );
	...
	{
		$model = new ExchangeGoods( "search" );
		$model->unsetAttributes( );
		if ( isset( $_GET['ExchangeGoods'] ) )
		{
			$model->attributes = $_GET['ExchangeGoods'];
		}
	...
	public function actionOperate( )
	{
		if ( Yii::app( )->request->isAjaxRequest && !empty( $_POST['id'] ) )
		{
			...
			{
				$models = ExchangeGoods::model( )->findAllByPk( $_POST['id'] );
				if ( $models )
				{
	...
	public function loadModel( )
	{
		if ( $this->_model === NULL )
		{
			if ( isset( $_GET['id'] ) )
			{
				$this->_model = ExchangeGoods::model( )->findbyPk( $_GET['id'] );
			}
			if ( $this->_model === NULL )
			{
	...
	{
		if ( isset( $_POST['ajax'] ) && $_POST['ajax'] === "exchange-goods-form" )
		{
