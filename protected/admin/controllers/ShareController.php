...
class ShareController extends Controller
{
	...
	{
		$form = new Form( "backend.views.share.formConfig" );
		$form->model = new Share( );
		...
		if ( $form->submitted( "btnsubmit" ) && $form->validate( ) )
		{
			...
			if ( $model->save( FALSE ) )
			{
				Yii::app( )->user->setFlash( "success", Share::model( )->modelName.( "“".$model."”已成功添加！" ) );
				$this->redirect( array(
					"view",
					"id" => $model->id
				) );
	...
	public function actionUpdate( )
	{
		...
		if ( $form->submitted( "btnsubmit" ) && $form->validate( ) )
		{
			...
			if ( $model->save( FALSE ) )
			{
				Yii::app( )->user->setFlash( "success", Share::model( )->modelName.( "“".$model."”已成功修改！" ) );
				$this->redirect( array(
					"view",
					"id" => $model->id
				) );
	...
	public function actionOperate( )
	{
		if ( Yii::app( )->request->isAjaxRequest && !empty( $_POST['id'] ) )
		{
			...
			{
				$shares = Share::model( )->findAllByPk( $_POST['id'] );
				if ( $shares )
				{
					foreach ( $sharesdA as $share )
					{
						$share->delete( );
					}
				}
	...
	public function loadModel( )
	{
		if ( $this->_model === NULL )
		{
			if ( isset( $_GET['id'] ) )
			{
				$this->_model = Share::model( )->findbyPk( $_GET['id'] );
			}
			if ( $this->_model === NULL )
			{
