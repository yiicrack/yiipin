<?php
class ArticleCategoryDropDownList extends CInputWidget
{
	...
	public function run( )
	{
		if ( isset( $this->htmlOptions['empty'] ) )
		{
			...
			unset( $this->htmlOptions['empty']['empty'] );
		}
		else
		{
			$data['0'] = "顶级分类";
		...
		$categorys = ArticleCategory::model( )->findAllByAttributes( array( "parent_id" => 0 ), array( "order" => "sortnum ASC" ) );
		foreach ( $categorys as $category )
		{
			if ( $category->ultimate == ArticleCategory::ULTIMATE_YES )
			{
				$data[$category->id] = $category->name;
			}
			else
			{
				$models = ArticleCategory::model( )->findAllByAttributes( array(
