...
function env_check(&$env_items) {
	foreach($env_items as $key => $item) {
		if($key == 'php') {
			...
			if(version_compare(PHP_VERSION, '5.1.6') <= 0 || version_compare(PHP_VERSION, '5.5.0') >= 0){
			    $env_items[$key]['current'] = 0;
			}
			...
			$tmp = extension_loaded('pdo') ? '已安装':0;
			$env_items[$key]['current'] = $tmp;
			unset($tmp);
		} elseif($key == 'pdo_mysql') {
			$tmp = extension_loaded('pdo_mysql') ? '已安装':0;
			$env_items[$key]['current'] = $tmp;
			unset($tmp);
		}elseif($key == 'curlversion') {
			$tmp = function_exists('curl_init') ? curl_version() : array();
			$env_items[$key]['current'] = empty($tmp['version']) ? 'noext' : $tmp['version'];
			unset($tmp);
		}elseif($key == 'mbstring') {
			$tmp = extension_loaded('mbstring') ? '已安装':0;
			$env_items[$key]['current'] = $tmp;
			unset($tmp);
//		}elseif($key == 'zend_optimizer') {
//			$tmp = extension_loaded('Zend Optimizer') || extension_loaded('Zend Guard Loader') ? '已安装':false;
//			$env_items[$key]['current'] = $tmp;
//			unset($tmp);
		}elseif($key == 'memcache') {
			$env_items[$key]['current'] = (extension_loaded("memcache") || extension_loaded("memcached")) ? '已安装' : 'noext';
			unset($tmp);
// 		}elseif($key == 'apc') {
// 			$env_items[$key]['current'] = extension_loaded("apc") ? '已安装' : 'noext';
// 			unset($tmp);
...
function show_footer($quit = true) {
	...
	echo <<<EOT
		<div class="footer">&copy;2012 <a href="http://www.yiipin.com/">武汉千比特网络工作室</a></div>
	</div>
</div>
</body>
</html>
...
function check_adminuser($username, $password, $email) {
	...
	@include UC_CONFIG;
	include ROOT_PATH.'./protected/vendors/uc_client/client.php';
	$error = '';
...
function install_uc_server() {
	...
	save_uc_config($config, ROOT_PATH.'./protected/config/ucenter.php');
