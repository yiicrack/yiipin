...
if($method == 'show_license') {
	...
	$default_ucapi = $bbserver.'/uc_server';
	...
	if($submit) {
		...
		if(!empty($license_key)) {
		    file_put_contents(ROOT_PATH.'./protected/config/license.php', "<?php die; ?>\n".strtoupper($license_key));
		}
		
		include_once ROOT_PATH.'./protected/vendors/uc_client/client.php';
...
// 				}
		...
		$db->query("REPLACE INTO {$tablepre}config (`key`, `value`) VALUES ('siteuniqueid', '".serialize($siteuniqueid)."')");

		$db->query("TRUNCATE {$tablepre}administrator");
		$salt = rand(10000, 99999);
		$password = md5(md5($password).$salt);
		$db->query("INSERT INTO {$tablepre}administrator (username, password, salt, email, this_login_time, this_login_ip, created) VALUES ('$username', '$password', '$salt', '$email', '".time()."', '127.0.0.1', unix_timestamp());");

		if($testdata) {
			install_testdata($username, $uid);
		}

		if(is_writeable(ROOT_PATH.'./protected/config/database.php')) {//TODO
			$configfile = @file_get_contents(ROOT_PATH.'./protected/config/database.php');
			$configfile = trim($configfile);
			$configfile = substr($configfile, -2) == '?>' ? substr($configfile, 0, -2) : $configfile;
			
			@file_put_contents(ROOT_PATH.'./protected/config/database.php', $configfile);
		}

		
		dir_clear(ROOT_PATH.'./protected/runtime');

		touch($lockfile);
		VIEW_OFF && show_msg('initdbresult_succ');

		if(!VIEW_OFF) {
			echo '<script type="text/javascript">document.getElementById("laststep").disabled=false;document.getElementById("laststep").value = \''.lang('install_founder_contact').'\';</script><script type="text/javascript">setTimeout(function(){window.location=\'index.php?method=ext_info&skip=1\'}, 2000);</script><iframe src="../" style="display:none"></iframe>'."\r\n";
			show_footer();
		}
	...
	preg_match('%mysql:host=(.+?);dbname=(.+$)%is', $config['connectionString'], $matches);
	$tablepre = $config['tablePrefix'];
	$db = new dbstuff;
	$db->connect($matches[1], $config['username'], $config['password'], $matches[2], DBCHARSET);
	$skip = getgpc('skip');
	if(empty($skip)) {
		upg_yiipin_stats();
	}
	...
	if(VIEW_OFF) {
		...
		echo '<script>setTimeout(function(){window.location=\'../\'}, 10000);</script>'.lang('auto_redirect').'</li>';
		echo '</ul></div>';
		show_footer();
	}
